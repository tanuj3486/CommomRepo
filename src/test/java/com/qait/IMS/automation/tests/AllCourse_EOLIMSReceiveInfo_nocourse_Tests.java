 ...
 * @author QAIT
 */
public class AllCourse_EOLIMSReceiveInfo_nocourse_Tests {
    ...
    public void Test() throws IOException {
		...
		if (Utilities.getYamlValue("IMSURLReceiveInformationnocourse.RunType").equalsIgnoreCase("Selected")){
			for (initialCounter=1; initialCounter<=Integer.parseInt(Utilities.getYamlValue("IMSURLReceiveInformationnocourse.TotalCource"));initialCounter++)
			{
	            TestListenerAdapter tla = new TestListenerAdapter();
	            TestNG testng = new TestNG();
	            testng.setTestClasses(new Class[]{EOLIMSReceiveInfo_nocourse_Test.class});
			...
			if (Utilities.getYamlValue("IMSURLReceiveInformationnocourse.RunType").equalsIgnoreCase("AllCources")){
