package com.qait.IMS.automation.pageobjects;
...
import com.qait.Domestic.automation.util.ReadWriteExcelFile;
import com.qait.Domestic.automation.util.Utilities;
import com.qait.IMS.automation.tests.AllCourse_EOLIMSReceiveInfo_nocourse_Tests;
import com.qait.IMS.automation.tests.EOLIMSReceiveInfo_nocourse_Test;;

public class EOLIMSReceiveInfo_nocourse_UI extends AbstractClass {

	WebDriver driver;

	ReadWriteExcelFile writeResult = new ReadWriteExcelFile(
			"LastRunReport/last/Result.xls");
	String course;

	public EOLIMSReceiveInfo_nocourse_UI(WebDriver driver) {
		super(driver);
		this.driver = driver;
	}

	@FindBy(xpath = "//form[@id='form9']/a")
	...
	@FindBy(id = "ctl00_ctl00_MasterContentPlaceHolder_NestedMasterContentPlaceHolder_Form_divMsg")
	protected WebElement countryErrorMessage;
	@FindBy(id = "ctl00_ctl00_MasterContentPlaceHolder_NestedMasterContentPlaceHolder_Form_btnCloseModal")
	protected WebElement countryErrorMessageOKButton;
	...
	protected WebElement streetaddress1TextBoxLabelOnStep1Page;		
	...
	@FindBy(xpath = "//textarea[@id='ctl00_ctl00_MasterContentPlaceHolder_NestedMasterContentPlaceHolder_Form_txtIntlCityState']")
	protected WebElement cityTextBoxOnStep1Page;	
	@FindBy(xpath = "//strong[contains(text(),'* City and Postal Code')]")
...
//	*********************
	...
	@FindBy(xpath = ".//textarea[@id='ctl00_ctl00_MasterContentPlaceHolder_NestedMasterContentPlaceHolder_Form_txt_PaymentInfo_CC_CityPostal_INTL']")
	...
	@FindBy(id = "ctl00_ctl00_MasterContentPlaceHolder_NestedMasterContentPlaceHolder_Form_ddl_PaymentInfo_CC_IntlForm_Country")
	protected WebElement textboxStateCreditCardOnPaymentInformationPage;
	@FindBy(xpath = "//strong[contains(text(),'*Country')]")
...
//	protected WebElement proceedtoFinalStepCreditCardOnPaymentInformationPage;
	...
	// @FindBy(xpath =
	// "//div[@id='ctl00_ctl00_MasterContentPlaceHolder_NestedMasterContentPlaceHolder_Form_div_MarketingContent']/span")
	...
	@FindBy(xpath = "//li[contains(text(),'Street Address is Required')]")
	protected WebElement address1isRequired;
	@FindBy(xpath = "//li[contains(text(),'City and Postal Code is Required')]")
	...
	protected WebElement pleaseEnterAddressInStreet;
            @FindBy(xpath = "//li[contains(text(),'enter your city and postal code')]")
...
//	protected WebElement okbuttonDisplayonPaymentInfoPage;
	...
	String email;
	String courseCost;
	int currentCourseUI = AllCourse_EOLIMSReceiveInfo_nocourse_Tests.initialCounter + 1;
	int ScenarioCount = AllCourse_EOLIMSReceiveInfo_nocourse_Tests.ScenarioCounter;
	...
	public void navigateToDomesticURLReciveInformationNoCource() {
		...
		Reporter.log(Utilities
				.logOutputFile("Click on Receive Information (No course) - Pass"));
		Assert.assertTrue(pennFosterCareerSchoolImage.isDisplayed());
		Reporter.log(Utilities
				.logOutputFile(" Penn Foster School Image Verify - Pass"));
		getScenarioFromYAML();
	...
	public int getCourseCountFromPannFosterPage() {
		...
		wait.until(ExpectedConditions.visibilityOfElementLocated(By
				.xpath("//select[@id='ctl00_ctl00_MasterContentPlaceHolder_NestedMasterContentPlaceHolder_Form_ddlProgram']")));
		coursecount = Integer.parseInt(courseDropDown.getAttribute("length"));
		Reporter.log(Utilities
				.logOutputFile("Total number of Course under Receive Information (No course) is --> "
						+ coursecount + " - Pass"));
	...
	public void selectCourseFromPannFosterPage(String Cource) {
		if (Utilities.getYamlValue(
			...
			WebElement OptionFordropdown = driver
					.findElement(By
							.xpath("//select[@id='ctl00_ctl00_MasterContentPlaceHolder_NestedMasterContentPlaceHolder_Form_ddlProgram']/option["
									+ currentCourseUI + "]"));
		...
		Reporter.log(Utilities
				.logOutputFile(" Select a course from dropdown - Pass"));
		...
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("ctl00_ctl00_MasterContentPlaceHolder_NestedMasterContentPlaceHolder_Form_divMsg")));					
		
		selectedCourseName = course;
		Assert.assertTrue(countryErrorMessage.getText().contains("We encountered the following errors:") && countryErrorMessage.getText().contains("Country is Required"));
		
		countryErrorMessageOKButton.click();
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("ctl00_ctl00_MasterContentPlaceHolder_NestedMasterContentPlaceHolder_Form_ddlCountry")));					

		System.out.println(Utilities.getYamlValue("IMSURLEnrollNownocourse.StudentInformation.Country"));
		select = new Select(driver.findElement(By.xpath("//select[@id='ctl00_ctl00_MasterContentPlaceHolder_NestedMasterContentPlaceHolder_Form_ddlCountry']")));
		...
		Reporter.log(Utilities
				.logOutputFile(" Click on Continue button - Pass"));
	}
...
//		try {
			...
			WebDriverWait wait = new WebDriverWait(driver, 50);
			wait.until(ExpectedConditions.visibilityOfElementLocated(By
					.xpath("//strong[contains(text(),'* First Name')]")));
			...
			Assert.assertTrue(courseNameHeadingOnStep1Page.getText()
					.equalsIgnoreCase(selectedCourseName));
			...
			Assert.assertTrue(streetaddress1TextBoxLabelOnStep1Page.isDisplayed());
...
//		}
	...
	public void fillFormStep1ForDomestic(String preFix, String Fname,
...
//			zipTextBoxOnStep1Page.sendKeys(Zip);
			...
			// //System.out.println("Error in validating all email message on Step1 page "
			// + e);
			okButtonValidationPopUp.click();
		}
		...
		try {
			...
			// //System.out.println("Error in validating all email required message on Step1 page "
			// + e);
			okButtonValidationPopUp.click();
		}
...
//		zipTextBoxOnStep1Page.sendKeys(Zip);
		...
		if (Utilities.getYamlValue(
				"IMSURLReceiveInformationnocourse.RunType")
				.equalsIgnoreCase("AllCources")) {
			writeResult.writeColInExcelFile("Result", course,
					currentCourseUI - 1, 0);
			writeResult.writeColInExcelFile("Result", Fname,
					currentCourseUI - 1, 1);
			writeResult.writeColInExcelFile("Result", Lname,
					currentCourseUI - 1, 2);
			writeResult.writeColInExcelFile("Result", Mail + preFix + "@"
					+ EmailExt, currentCourseUI - 1, 3);
			writeResult.writeColInExcelFile("Result", Phone,
					currentCourseUI - 1, 4);
			writeResult.writeColInExcelFile("Result", Address,
					currentCourseUI - 1, 5);
			writeResult.writeColInExcelFile("Result", City,
					currentCourseUI - 1, 6);
			writeResult.writeColInExcelFile("Result", Zip, currentCourseUI - 1,
					7);
		} else {
			if (Utilities.getYamlValue(
					"IMSURLReceiveInformationnocourse.RunType")
				...
				writeResult.writeColInExcelFile("Result", City,
						currentCourseUI - 1, 6);
				writeResult.writeColInExcelFile("Result", Zip,
						currentCourseUI - 1, 7);
			}
		}
	}
	...
	public void verifyThnakYouPageAfterStep1FormPage() {
		WebDriverWait wait = new WebDriverWait(driver, 50);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("innerboxThankYou")));
		
		

		Assert.assertTrue(thnakyouPageAfterStep1FormPage.isDisplayed());
		Reporter.log(Utilities
				.logOutputFile("Verify thankyou page displayed  - PASS "));
	...
	public void verifyPersonalInformationPage() {
		try {
			...
			Assert.assertTrue(courseMainNameHeadingOnStep1Page.getText().equalsIgnoreCase(selectedCourseName));
			Assert.assertTrue(firstNameTextBoxOnStep1Page.isDisplayed());
			Assert.assertTrue(firstNameTextBoxOnStep1Page.getAttribute("value").equals(Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.FirstName")));
			...
			Assert.assertTrue(firstNameTextBoxLabelOnStep1Page.isDisplayed());
			Assert.assertTrue(lastNameTextBoxOnStep1Page.isDisplayed());
			Assert.assertTrue(lastNameTextBoxOnStep1Page.getAttribute("value").equals(lname));

			Assert.assertTrue(lastNameTextBoxLabelOnStep1Page.isDisplayed());
			Assert.assertTrue(emailTextBoxOnStep1Page.isDisplayed());
			Assert.assertTrue(emailTextBoxLabelOnStep1Page.isDisplayed());
			Assert.assertTrue(emailTextBoxOnStep1Page.getAttribute("value").equalsIgnoreCase(email+ "@"	+ Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.EmailExt")));

			
			Assert.assertTrue(reTypeEmailTextBoxOnStep1Page.isDisplayed());
			Assert.assertTrue(reTypeEmailTextBoxOnStep1Page.getAttribute("value").equalsIgnoreCase(email+ "@"	+ Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.EmailExt")));

			Assert.assertTrue(reTypeEmailTextBoxLabelOnStep1Page.isDisplayed());
			Assert.assertTrue(primaryPhoneTextBoxOnStep1Page.isDisplayed());
			Assert.assertTrue(primaryPhoneTextBoxOnStep1Page.getAttribute("value").equals(Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.PrimaryPhone")));

			
			Assert.assertTrue(primaryPhoneTextBoxLabelOnStep1Page.isDisplayed());
			Assert.assertTrue(address1TextBoxOnStep1Page.isDisplayed());
			Assert.assertTrue(address1TextBoxOnStep1Page.getAttribute("value").equals(Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.Address")));
			...
			Assert.assertTrue(address2TextBoxLabelOnStep1Page.isDisplayed());
			Assert.assertTrue(cityTextBoxOnStep1Page.isDisplayed());
			Assert.assertTrue(cityTextBoxOnStep1Page.getAttribute("value").equals(Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.City")));
			...
			String Address, String City, String State, String Zip , String DOBmonth, String DOBday, String DOByear)	
{
		...
		try {
			...
			System.out
					.println("Error in validating all empty message on Step1 page "
							+ e);
			okButtonValidationPopUp.click();
		}
		...
		try {
			...
			System.out
					.println("Error in validating all first name message on Step1 page "
							+ e);
			okButtonValidationPopUp.click();
		}
		...
		try {
			...
			System.out
					.println("Error in validating all last name message on Step1 page "
							+ e);
			okButtonValidationPopUp.click();
		}
		...
		try {
			...
			System.out
					.println("Error in validating all email required message on Step1 page "
							+ e);
			okButtonValidationPopUp.click();
		}
		...
		lname = Lname;
		email = Mail + preFix;
		if (Utilities.getYamlValue("IMSURLEnrollNownocourse.RunType")
				.equalsIgnoreCase("AllCources")) {
			writeResult.writeColInExcelFile("Result", course,
					currentCourseUI - 1, 0);
			writeResult.writeColInExcelFile("Result", Fname,
	...
	public void  verificationPaymentInformationPage()	
	{
		...
		if(PaymentScenario.contains("Monthly Payment") || PaymentScenario.contains("Pay By Mail")){
			
			if (radioButton.size() == 1){
			    if (fullPaymentPlanRadioOnPaymentInformationPage.isDisplayed()){
				PaymentScenario = "Full Payment; Credit or Debit Card";
				AllCourse_EOLIMSReceiveInfo_nocourse_Tests.ScenarioCounter = ScenarioCount - 1;
		...
		EOLIMSReceiveInfo_nocourse_Test.PayScenarioGet = PaymentScenario;		
		
		System.out.println("PaymentScenario : "+PaymentScenario);
		splitScenario();
	}
...
//		#################  Full Payment Mode  #################
		...
		if (parts[0].trim().equalsIgnoreCase("Full Payment")){
		    ...
		    Assert.assertTrue(str.contains("Please select your payment method:") &&  
			str.contains("Credit / Debit Card")  &&
			str.contains("Payment Information:") &&	str.contains("Today's Payment:"));
			...
			Assert.assertTrue(textboxFirstNameCreditCardOnPaymentInformationPage.getAttribute("value").equalsIgnoreCase(Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.FirstName")));
			...
			Assert.assertTrue(textboxAdderss1CreditCardOnPaymentInformationPage.getAttribute("value").equalsIgnoreCase(Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.Address")));
			...
			Assert.assertTrue(textboxCityCreditCardOnPaymentInformationPage.getAttribute("value").equalsIgnoreCase(Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.City")));
			...
			try{
	                			...
	                			wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='ctl00_ctl00_MasterContentPlaceHolder_NestedMasterContentPlaceHolder_Form_btnCloseModal']")));
...
//                                textboxZipCreditCardOnPaymentInformationPage.sendKeys(zipCode);
			...
			String CreditCardNumber = Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.CreditCardNumber");
			String CVV = Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.CVV"); 
...
//			    writeResult.writeColInExcelFile("Result", "Full Payment; Credit or Debit Card", currentCourseUI-1, 8);
...
//				Enter date
				...
				wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("ctl00_ctl00_MasterContentPlaceHolder_NestedMasterContentPlaceHolder_Form_btn_PaymentFrequency_Submit")));
...
//				    writeResult.writeColInExcelFile("Result", "Full Payment; Credit or Debit Card", currentCourseUI-1, 8);
	...
	public void verifyElementsOnReviewAndSubmitPage(String preFix, String Fname, String Lname, String Mail, String EmailExt, String Phone,
			String Address, String City, String State, String Zip , String DOBmonth, String DOBday, String DOByear)	

{
		...
		Assert.assertTrue(PersonalInformation.contains("Personal Information") &&	  PersonalInformation.contains("Personal Information") &&
			PersonalInformation.contains("First Name:")      &&   PersonalInformation.contains("Last Name:")           &&
			PersonalInformation.contains("Street Address:")         &&	  PersonalInformation.contains("City and Postal Code:")                &&
		...
		if (PaymentScenario.contains("By 14 days")){
		    Assert.assertTrue(ProgramInformation.contains("Program Information")    &&	ProgramInformation.contains("Program:")               &&
				ProgramInformation.contains("Program Price:")     &&	ProgramInformation.contains("Initial Payment:")       &&
				ProgramInformation.contains("Bi-Weekly Payment:")   &&	ProgramInformation.contains("# of Bi-Weekly Payments:") &&
		...
		if (PaymentScenario.contains("Credit or Debit Card")){
		    Assert.assertTrue(PaymentInformation.contains("Amount Charged Today")     &&	PaymentInformation.contains("Payment Information") &&
		...
		if (PaymentScenario.equalsIgnoreCase("Checking or Savings Account")){
		...
		List<WebElement> check = driver.findElements(By.cssSelector("div[class^='column review-right']"));
			...
			m = check.get(4);
			Assert.assertTrue(m.getText().contains("IND"));	
			...
			m = check.get(7);			
			Assert.assertTrue(m.getText().contains(Utilities.getYamlValue("IMSURLEnrollNownocourse.StudentInformation.DOBmonth")+"/"+Utilities.getYamlValue("IMSURLEnrollNownocourse.StudentInformation.DOBday")+"/"+Utilities.getYamlValue("IMSURLEnrollNownocourse.StudentInformation.DOByear")));
	
			m = check.get(8);
			Assert.assertTrue(m.getText().contains(course));
				
			
			if (PaymentScenario.equalsIgnoreCase("Full Payment")){
				...
				m = check.get(14);
				Assert.assertTrue(m.getText().contains("0"));
				
				m = check.get(15);
				Assert.assertTrue(m.getText().contains("$0.00"));
				
				if (PaymentScenario.equalsIgnoreCase("Credit or Debit Card")){
			...
			if (PaymentScenario.contains("Monthly Payment")){
				...
				Assert.assertTrue(courseCost.contains(m.getText()));
			    	...
			    	m = check.get(10);
			    	System.out.println("$"+MinCost+".00");
			    	System.out.println(m.getText());
				Assert.assertTrue(m.getText().contains("$"+MinCost+".00"));
				
				
				if (PaymentScenario.contains("By 14 days")){
				    	System.out.println(ByWeeklyPay);
				    	m = check.get(11);
					Assert.assertTrue(m.getText().contains(ByWeeklyPay));
				    
				}else{
				    
				    	m = check.get(11);
					Assert.assertTrue(m.getText().contains("$"+MaxCost+".00"));
				}
				

				if (PaymentScenario.contains("By 14 days")){
				    m = check.get(12);
					Assert.assertTrue(m.getText().contains(""+MonthCount*2));
				    
				}else{
				    m = check.get(12);
					Assert.assertTrue(m.getText().contains(""+MonthCount));
				...
				m = check.get(13);
				Assert.assertTrue(m.getText().contains("$"+FinalPayCost+".00"));
			    
//				if (PaymentScenario.contains("By Monthly; Credit or Debit Card")){
					if (PaymentScenario.contains("Credit or Debit Card")){
...
//
//			m = check.get(9);			
//			Assert.assertTrue(m.getText().contains(Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.DOBmonth")+"/"+Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.DOBday")+"/"+Utilities.getYamlValue("IMSURLReceiveInformationnocourse.StudentInformation.DOByear")));
//	
//			m = check.get(10);
//			Assert.assertTrue(m.getText().contains(course));
//				
...
//				
//				
//				if (PaymentScenario.equalsIgnoreCase("Checking or Savings Account; Savings")){
//				    
//				    	m = check.get(16);
//					Assert.assertTrue(courseCost.contains(m.getText()));
//					
//					m = check.get(17);
//					String accountnumber = Utilities.getYamlValue("IMSURLReceiveInformationnocourse.AccountNumber");
//					Assert.assertTrue(m.getText().contains("XXXXX") && m.getText().contains(accountnumber.subSequence(accountnumber.length()-4, accountnumber.length())));
//					
//					m = check.get(18);
//					Assert.assertTrue(m.getText().contains("$0.00"));
//				    
//				}
//				
//				if (PaymentScenario.equalsIgnoreCase("Checking or Savings Account; Checking")){
//				    
//				    	m = check.get(16);
//					Assert.assertTrue(courseCost.contains(m.getText()));
//					
...
//
//				
//						if (PaymentScenario.contains("Checking or Savings Account")){
//				    
//					m = check.get(16);
//					Assert.assertTrue(m.getText().contains("$"+MinCost+".00"));
//					
	...
	public boolean acceptTermandConditionOnReviewAndSubmitPage() {
		...
		try {
			...
			if (Utilities.getYamlValue(
					"IMSURLReceiveInformationnocourse.RunType")
					.equalsIgnoreCase("AllCources")) {
				writeResult.writeColInExcelFile("Result", "ID not generated",
						currentCourseUI - 1, 9);
				...
				if (Utilities.getYamlValue(
						"IMSURLReceiveInformationnocourse.RunType")
						.equalsIgnoreCase("Selected")) {
					writeResult.writeColInExcelFile("Result",
							"ID not generated", currentCourseUI - 1, 9);
				}
		...
		if (m == 1) {
			try {
				...
				try {
					Reporter.log(Utilities
							.logOutputFile(" Final Student ID is "
									+ getStudentIDFromCongratulationsPage2
											.getText() + " - Pass"));
					...
					if (Utilities.getYamlValue(
							...
							.equalsIgnoreCase("AllCources")) {
						writeResult.writeColInExcelFile("Result",
								getStudentIDFromCongratulationsPage2.getText(),
								currentCourseUI - 1, 9);
