...
public class CreateCoursePage extends BaseWebComponent {
	...
	private String radio_createNewCourse_id = "createNewCourse";
	...
	public void selectCreateNewCourseRadioBtnAndContinue() {
		hardWait(2);
		waitAndLocateElementById(radio_createNewCourse_id).click();
		// waitAndLocateElementByXpath(radio_createNewCourse_xpath).click();
		// findElementByXpath(radio_createNewCourse_xpath).click();
		hardWait(1);
		waitAndLocateElementByLinkText(btn_continue_linktext).click();
	...
	public void getCourseKey() {
		...
		courseKey = regUrl.substring(regUrl.lastIndexOf('/') + 1);// Split URL
																	// to get
																	// course
																	// key
		courseKey = removeHyphens(courseKey);// hyphens(-) are removed from the
												// retrieved course key
	}
	...
	public void saveCourseNameToFile() {
		...
		logPassMessage("New course created: Course name- " + courseName
